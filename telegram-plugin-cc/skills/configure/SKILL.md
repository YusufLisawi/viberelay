...
1. **Token** — check `${TELEGRAM_STATE_DIR:-~/.claude/channels/telegram}/.env` for
   `TELEGRAM_BOT_TOKEN`. Show set/not-set; if set, show first 10 chars masked
   (`123456789:...`).

2. **Access** — read `${TELEGRAM_STATE_DIR:-~/.claude/channels/telegram}/access.json` (missing file
...
2. `mkdir -p ${TELEGRAM_STATE_DIR:-~/.claude/channels/telegram}`
3. Read existing `.env` if present; update/add the `TELEGRAM_BOT_TOKEN=` line,
   preserve other keys. Write back, no quotes around the value.
4. `chmod 600 ${TELEGRAM_STATE_DIR:-~/.claude/channels/telegram}/.env` — the token is a credential.
